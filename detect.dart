...
class Predictor {
  Future loadModel() async{
    Tflite.close();
    await Tflite.loadModel(model: "assets/converted_mnist_model.tflite",
  ...
  Future<List> processPoints(List<Offset> points) async {
    ...
    final vircanvas = Canvas(recorder,
        Rect.fromPoints(Offset(0, 0), Offset(canvasSize, canvasSize)));

    vircanvas.drawRect(
        Rect.fromLTWH(0, 0, canvasSize, canvasSize), blackback);

    void paint(Canvas canvas, Size size) {
      for (int i = 0; i < points.length - 1; i++) {
        if (points[i] != null && points[i + 1] != null) {
          canvas.drawLine(points[i], points[i + 1], vcanpaint);
        }
      }
    }

    paint(vircanvas, Size(200, 200));
